...
class GAN:
    ...
    def __init__(self, options):
        ...
        self.gbn0 = ops.batch_norm(name='gbn0')
        self.gbn1 = ops.batch_norm(name='gbn1')
        self.gbn2 = ops.batch_norm(name='gbn2')
    ...
    def build_model(self):
        ...
        t_real_caption = tf.placeholder('float32', [self.options['batch_size'], 15, 400], name = 'real_caption_input')
        #t_z = tf.placeholder('float32', [self.options['batch_size'], 100])

        fake_image, cnn_feature = self.generator(t_real_caption)

        disc_real_image, disc_real_image_logits = self.discriminator(t_real_image, cnn_feature)
        disc_wrong_image, disc_wrong_image_logits = self.discriminator(t_wrong_image, cnn_feature, reuse=True)
        disc_fake_image, disc_fake_image_logits = self.discriminator(fake_image, cnn_feature, reuse=True)
    ...
    def discriminator(self, image, cnnfeature, reuse=False):
        with tf.variable_scope("discriminator") as scope:
            ...
            reduced_text_embeddings = ops.lrelu(ops.linear(cnnfeature, self.options['t_dim'], 'd_embedding'))
